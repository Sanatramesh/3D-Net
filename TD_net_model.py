...
class TDNet_VGG11(TDNet):
    ...
    def train_epoch(self, data):
        ...
        feat_left  = self.sess.run( self.vgg.pool3, feed_dict = {img_pl : left_cam_data} )
        # get the feature maps for right camera images
        feat_right = self.sess.run( self.vgg.pool3, feed_dict = {img_pl : right_cam_data} )
        # element-wise multiplication of the feature maps
        feature_maps = tf.multiply( feat_left, feat_right )
...
class ModelTraining:
    ...
    def train_model(self):
        ...
        for i in range( self.no_epochs ):
            ...
            batch_idx = next_batch( self.no_data, self.batch_size )
            validation_data = read_data( self.train_data_files[ batch_idx ] )
...
def main():
    ...
    net = TDNet_VGG11([480, 640], [480,640], [60, 80, 256], 'data/vgg16.npy', learning_rate = 1e-4)
    net.build_model()
    ...
    print (data_shuffled[:5])

    train_data_files = data_shuffled[len(data_shuffled)//5:]
    test_data_files  = data_shuffled[:len(data_shuffled)//5]

    print (len(train_data_files), len(test_data_files))

    train = ModelTraining(net, data_files)
    train.train_model()
